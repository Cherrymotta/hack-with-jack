...
contract('Vesting', async (accounts) => {
    ...
    beforeEach(async() => {
        ...
        //seedStart         = Math.round(Date.now() / 1000);
    ...
    it('Create vesting schedules', async () => {    
        ...
        for(i in seedRoundArray) {
            ...
            amount = parseInt(investArray[1]);
            amountAfterCliff = Math.round(amount * seedCliffPercent / 1000); // Рассчитать на основе общей суммы по процентам, а от общей суммы отнять эту
            amount = amount - amountAfterCliff;
            ...
            await vesting.addInvestor(
                ...
                amountAfterCliff,    
                seedPhaseId
                , { from: trader0 } // owner of the contract
                // { from: trader1 } // admin of the contract
            );
    ...
    it('Get vesting schedule by address and index', async () => {
        ...
        for(i in investorsArray) {
            ...
            for (ind = 0; ind < VScount.toNumber(); ind++) {
                ...
                console.log(`${i}). Vesting schedule N${ind} of beneficiary: ${investor} - `, BigInt(vestingSchedule.amountTotal));
            }
        }; 
    ...
    //     console.log('Balance of investor must be 0');
    ...
    //             console.log('  - ReleasableAmount: ', parseInt(ReleasableAmount));
    //             await vesting.release(vestingScheduleId, parseInt(ReleasableAmount));
    //             console.log(`${i}). Released schedule ID ${vestingScheduleId} for investor: ${investor}. Amount = `, parseInt(ReleasableAmount));  
    ...
    //     await changeTime(seedSlice); // проматываем первый слайс
    ...
    //         await changeTime(seedSlice); // проматываем следующий слайс
    ...
    //     const VestingTotalAmount = await vesting.getvestingTotalAmount();
    //     console.log('vesting total amount: ', VestingTotalAmount.toNumber());
