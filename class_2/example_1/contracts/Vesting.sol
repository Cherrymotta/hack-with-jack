...
contract Vesting is ReentrancyGuard, AccessControl {
    ...
    struct InvestorData {
        ...
        uint256 amountTotal; // total amount of tokens to be released at the end of the vesting except for amount paid after the cliff
    ...
    struct VestingSchedule {
        ...
        uint256 amountTotal; // total amount of tokens to be released at the end of the vesting except for percentages after the cliff
    ...
    modifier onlyIfNotRevoked(bytes32 investorDataId) {
        require(investorsData[investorDataId].amountTotal > 0);
        _;
    }
        ...
        returns (VestingSchedule[] memory)
    {
        ...
        for (uint256 i = 1; i <= vestingSchedulesCount; i++) {
            schedulesArray[i - 1] = getVestingScheduleByAddressAndIndex(
                _investor,
                i
            );
        }
     ...
     * @param _amountAfterCliff amount of tokens to be payd after cliff
     * @param _phaseID ID of the vesting phase
     */
    function addInvestor(
        address _investor,
        uint256 _amount,
        uint256 _amountAfterCliff,
        uint256 _phaseID
    ) public onlyAdminOrSA {
    ...
    /**
     * @notice Cancels an existing schedule by resetting the amountTotal
        ...
        onlyAdminOrSA
    {
        ...
        vestingTotalAmount =
            vestingTotalAmount -
            (investorData.amountTotal - investorData.released);
        investorData.amountTotal = 0;
    ...
    function changeInvestorSchedule(
        ...
        if (_amount == 0) {
            _amount = investorData.amountTotal;
        }
        if (_released == 0) {
            _released = investorData.released;
        }
        if (_amountAfterCliff == 0) {
            _amountAfterCliff = investorData.amountAfterCliff;
        }
        if (_phaseID == 0) {
        ...
        investorData.amountTotal = _amount;
        ...
        returns (uint256)
    {
        ...
        if (
            (currentTime < vestingPhase.cliff) ||
            (investorData.amountTotal == 0) // If cliff not finished or total amount = 0 (schedule was canceled)
        ) {
            return 0;
        } else if (currentTime >= vestingPhase.cliff + vestingPhase.duration) {
            // If vesting period finished
            return investorData.amountTotal - investorData.released;
        } else {
            ...
            uint256 vestedAmount = (investorData.amountTotal * vestedSeconds) /
                vestingPhase.duration;
            if (investorData.cliffPaid) {
